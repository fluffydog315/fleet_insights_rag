...
def load_docs_with_tracking(tracker: DocumentTracker):
    ...
    if num_removed > 0:
        print(f"🗑️  Removed {num_removed} deleted files from tracker")
...
def main():
    ...
    if config.is_incremental_enabled():
        ...
        if not tracker.check_embedding_mode_compatibility(config.embedding_mode):
            print(f"⚠️  Embedding mode changed: {tracker.embedding_mode} → {config.embedding_mode}")
            print(f"⚠️  Different embedding models have different dimensions.")
            print(f"⚠️  Rebuilding entire index from scratch...")
            print()
        ...
        print("⚠️  Incremental indexing is disabled. Rebuilding entire index...")
        print()
